...
CInstall::CInstall(QSettings &acfg, const QCommandLineParser &args, const QString &cfgfile) noexcept
    ...
    helpBackdrop("/usr/share/chimbalix-installer/data/backdrop-textbox.png")
{
    ...
    TIPSTAGE4 = appConf.value("TIP_STAGE_4").toString();
...
void CInstall::startup()
{
    proc.log(__PRETTY_FUNCTION__, MProcess::LOG_MARKER);
    connect(gui.pushClose, &QPushButton::clicked, this, &CInstall::close);
    connect(gui.progInstall, &QProgressBar::valueChanged, this, &CInstall::progressUpdate);
    // Lambda slots
    ...
    setupkeyboardbutton();
    connect(gui.pushSetKeyboard, &QPushButton::clicked, this, &CInstall::runKeyboardSetup);
...
void CInstall::splashSetThrobber(bool active) noexcept
{
    if (active) {
        if (throbber) return;
...
void CInstall::setupAutoMount(bool enabled)
{
    proc.log(__PRETTY_FUNCTION__, MProcess::LOG_MARKER);
...
void CInstall::processNextPhase() noexcept
{
    try {
...
void CInstall::pretendNextPhase() noexcept
{
    if (phase == PH_READY) {
...
void CInstall::manageConfig(enum ConfigAction mode) noexcept
{
    if (mode == CONFIG_SAVE) {
...
int CInstall::showPage(int curr, int next) noexcept
{
...
void CInstall::pageDisplayed(int next) noexcept
{
    ...
    case Step::PROGRESS:
        gui.textHelp->setText("<p><b>" + tr("Installation in Progress") + "</b><br/>"
            + tr("%1 is installing. For a fresh install, this will probably take 3-30 minutes, depending on the speed of your system and the size of any partitions you are reformatting.").arg(PROJECTNAME)
            + "</p><p>"
...
void CInstall::gotoPage(int next) noexcept
{
    gui.pushBack->setEnabled(false);
...
bool CInstall::eventFilter(QObject *watched, QEvent *event) noexcept
{
...
void CInstall::changeEvent(QEvent *event) noexcept
{
...
void CInstall::closeEvent(QCloseEvent *event) noexcept
{
...
void CInstall::reject() noexcept
{
    if (checkmd5) checkmd5->halt();
}

void CInstall::abortUI(bool manual, bool closing) noexcept
{
...
void CInstall::abortEndUI(bool closenow) noexcept
{
    if (closenow) qApp->exit(EXIT_FAILURE); // this->close() doesn't work.
        ...
        connect(gui.pushNext, &QPushButton::clicked, this, &CInstall::close);
...
void CInstall::progressUpdate(int value) noexcept
{
    int newtip = 0;
    if (ixTipStart < 0) {
    ...
    case 4:
        gui.textTips->setText(tr("<p>For more information please visit: %1</p>%2").arg(PROJECTURL, TIPSTAGE4));
        break;

    default:
        gui.textTips->setText(tr("<p><b>Special Thanks</b><br/>Thanks to everyone who has chosen to support %1 with their time, money, suggestions, work, praise, ideas, promotion, and/or encouragement.</p>").arg(PROJECTNAME));
        break;
    }
}

void CInstall::setupkeyboardbutton() noexcept
{
...
void CInstall::runKeyboardSetup() noexcept
{
